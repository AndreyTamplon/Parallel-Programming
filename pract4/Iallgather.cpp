...
void GameOfLifeSimulation(bool* subfield, int* sendcounts, int size, int rank, int columns)
{
	...
	bool* stop_flags;
	bool* stop_flags_matrix;
	while (true)
	{
		MPI_Request send_to_above, send_to_below, receive_from_above, receive_from_below, stop_flags_matrix_request;
		...
		if (i > 0)
		{
			stop_flags = new bool[i];
			assert(stop_flags);
			SetStopFlags(stop_flags, previous_states, subfield, subfield_rows, columns);
			stop_flags_matrix = new bool[i * size];
			assert(stop_flags_matrix);
			MPI_Iallgather(stop_flags, i, MPI_C_BOOL, stop_flags_matrix,
						   i, MPI_C_BOOL, MPI_COMM_WORLD, &stop_flags_matrix_request);
		}
		...
		if(i > 0)
		{
			MPI_Wait(&stop_flags_matrix_request, MPI_STATUS_IGNORE);
